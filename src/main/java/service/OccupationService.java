...
public class OccupationService  {
		...
		try {
			...
			if (rs.next()) {
				return new Occupation(rs.getInt("ID"), rs.getDate("Date"), service.findById(rs.getInt("IDCrenom"))
						, service2.findById(rs.getInt("IDSalle")),client.findById(rs.getInt("Idclient")),rs.getBoolean("Validation"));
			}
	...
	public List<Occupation> findBySalleDate(int salle, java.util.Date date) {
		...
		try {
			...
			ClientService client = new ClientService();
			while (rs.next()) {
				occupations.add(new Occupation(rs.getInt("ID"), rs.getDate("Date"), service.findById(rs.getInt("IDCrenom"))
